    ...
    header: () => {
      ...
      if (linkedin) {
        ...
        contactSections.push(
          `\\href{${escapeLaTeX(
            linkedinUrl
          )}}{\\raisebox{-0.2\\height}\\faLinkedin\\ \\underline{${escapeLaTeX(
            linkedinText
          )}}}`
        );
      }
      ...
      if (github) {
        ...
        contactSections.push(
          `\\href{${escapeLaTeX(
            githubUrl
          )}}{\\raisebox{-0.2\\height}\\faGithub\\ \\underline{${escapeLaTeX(
            githubText
          )}}}`
        );
      }
  ...
  \\resumeSubHeadingListEnd`;
    ...
    projects: () => {
      ...
      const projectItems = projects.items
        .map((project) => {
          ...
          return `    \\projectEntry{${createHyperlink(
            project.title || "",
            project.url || ""
          )}}{}
    ...
    ${
      project.technologies?.length
        ? `\\resumeItemListStart
      \\resumeItem{Technologies: ${project.technologies
    ...
    volunteer: () => {
      ...
      const volunteerItems = volunteer.items
        .map((vol) => {
          ...
          const endDate = vol.dates?.is_current
            ? "Present"
            : formatDate(vol.dates?.end);
          const dateString =
            startDate || endDate
...
\\section{${escapeLaTeX(interests.section_title || "Interests")}}
  \\resumeSubHeadingListStart
    \\resumeParagraph{${validItems
      .map((interest) => escapeLaTeX(interest))
      .join(" | ")}}
  \\resumeSubHeadingListEnd`;
    },
    ...
    research: () => {
      ...
      const researchItems = research.items
        .map((item) => {
          ...
          return `      \\resumeParagraph{${escapeLaTeX(
            item.description || ""
          )}}`;
        })
        .filter(Boolean)
...
${customItems}
  ...
  const sectionOrder =
    cvData.cv_template.metadata?.section_order || defaultSectionOrder;
...
// Export the function
module.exports= { generateCVLatexTemplate2_new };
